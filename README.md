...
**Target: equal thermodynamic length per step.** We want each integration step to contribute the same amount to a **squared thermodynamic length** Δℓ². We want each step to contribute the same Δℓ². Over a short timestep Δ*t*, displacement scales as Δ*x* ∼ *a* Δ*t*² (*a* = *F*/*m*); a curvature-weighted cost scales as (Δ*t*)² S̄. Dividing by *k*ₑ*T* gives a dimensionless squared length. So Δℓ² ∝ (Δ*t*² S̄)/(*k*ₑ*T*). Setting this constant and solving for Δ*t* gives
