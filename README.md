# ETL: Equal Thermodynamic-Length Adaptive Control for ReaxFF MD

This repository contains the **Equal Thermodynamic-Length (ETL)** methodology for adaptive control of molecular dynamics in ReaxFF (SiO₂ and related systems). It includes the Python controller, LAMMPS-driven suite runner, and the full methodology report with derivations and benchmark results. Full methodology and results are documented below.
...
**Fixed vs. updated Δ*l*.** Defining Δ*l* once from the initial configuration keeps the target "length per step" constant and avoids drift: every step is compared to the same budget. Updating Δ*l* during the run (e.g. from a running average of S̄) could in principle let the target length track a changing difficulty, but would redefine the budget over time and could make adaptation inconsistent or biased toward later phases. In our implementation we keep Δ*l* fixed so that all adaptation comes from the local S̄ and *T*; the clamp [*dt*_min, *dt*_max] then bounds how far dt can deviate from the moderate baseline. This choice is conservative and may limit how much dt can grow in very calm regions if the initial calibration was done in a stiffer state; the trade-off is simplicity and a single, well-defined length scale for the whole run.
...
**Link to the traversed length and the cap.** Each step contributes a fixed squared thermodynamic length Δℓ² from the integration (dt term). The **force error** due to finite QEq tolerance also contributes to the "length" or cost of the step; we keep that contribution bounded by allocating a **fraction α_qeq** of the per-step budget to it, so that over the whole trajectory the total squared length from QEq error stays at most a fraction α_qeq of the total squared length traversed. What matters is *how we express* the QEq contribution in the same units as Δℓ².

**Why the QEq term has the form *err* × (Δ*t*)²/(*k*ₑ*T*).** The force error *err* (mass-weighted squared force error) has dimensions of force²/mass. It is not by itself a squared length. The **physical reason** to multiply *err* by (Δ*t*)² is that the force error acts over the **whole step** of length Δ*t*: if we think of the error as an error in acceleration, its effect on displacement over the step scales like (error in *a*) × Δ*t*², so the *squared* impact of the error over that step scales like *err* × (Δ*t*)². So *err* × (Δ*t*)² is the natural "squared cost" of the QEq error *for that step*—the same error in a longer step has a larger effect. Dividing by *k*ₑ*T* then does two things: it makes the quantity dimensionless (so we can compare it to Δℓ²), and it normalizes by the thermal scale in the same way as the dt term, so the measure is in "information" or thermal-fluctuation units. Thus the QEq contribution to squared thermodynamic length is ∝ *err* × (Δ*t*)²/(*k*ₑ*T*). We require this to be at most α_qeq × Δℓ² (the fraction of the per-step budget we allow for QEq). So *err* × (Δ*t*)²/(*k*ₑ*T*) ≤ α_qeq Δℓ², which gives *err* ≤ α_qeq Δℓ² × (*k*ₑ*T*)/(Δ*t*)². The right-hand side is the **cap**:
...
**Early exit.** When *s* ≥ *t_ps*, the full schedule has been traversed; we **exit** the run even if the simulated time is below *t_ps*. That is the source of wall-time savings for ramp-enabled runs. For hat and milder schedules, only the cold phases are easy; in the hot phase S̄ ≥ S̄_ref and speed_factor = 1, so we exit after ~7.9–8.4 ps simulated time when the virtual clock reaches 10 ps.
...
![Fig. 2: Virtual schedule clock — simulated time vs virtual time; early exit when s reaches schedule duration.](docs/figures/fig02_virtual_clock.png)
...
### 2.2 Decoupling Timestep from QEq Noise (S̄_tight)

When adaptive QEq is enabled, we use a looser tolerance in calm phases to save cost; the forces returned are noisier. The force power **S̄** is computed from these forces, so S̄ would be **inflated** by QEq error and would shrink Δ*t* unnecessarily if fed into the dt formula. We **decouple** the S̄ used **for the dt formula** from the operating tolerance by using **S̄_tight**—force power from forces at **tight** QEq tolerance—computed during **sentinel calibration** and cached until the next calibration. The timestep controller then uses S̄_tight so the curvature signal reflects the true stiffness, not QEq noise. The overall ETL control loop (trajectory → S̄ and *T* → dt; QEq budget → *tol*; optional virtual clock → early exit) is summarized in **Figure 1**.

![Fig. 1: ETL control loop (dt, QEq budget, optional ramp).](docs/figures/fig01_control_loop.png)

*Figure 1. ETL control loop. **Top row:** Forces (F) and target temperature (T) → force power S̄ → dt formula → integration timestep Δt. **Bottom row:** QEq cap (∝ 1/Δt²) sets tolerance *tol*; virtual clock advances by speed_factor and triggers early exit when the schedule is complete.*

### 2.3 Temperature Schedules
...
![Fig. 3: Wall time (s) by case and schedule, 576 and 1500 atoms.](docs/figures/fig03_wall_time.png)

*Figure 3. Wall time by case (ref, mod, agg, dt, dt+r, qeq, full) for hat and constant T schedules; left panel 576 atoms, right panel 1500 atoms.*
...
![Fig. 4: Speedup vs moderate baseline (0.25 fs), 576 and 1500 atoms.](docs/figures/fig04_speedup_vs_moderate.png)
...
- **tol**: From the cap formula (cap ∝ 1/dt²), when dt is **large** (calm phase) the cap is **smaller**, so we require smaller force error and thus **tighter** *tol*; when dt is **small** (stiff phase) the cap is **larger**, so we allow **looser** *tol*. In practice, adaptive-QEq runs keep *tol* in a narrow range (e.g. 4–8×10⁻³); the cap and tol_max rarely allow much looser tolerance, so QEq iteration count does not change dramatically from chunk to chunk (**hypothesis H2**: limited *tol* range limits QEq savings). Explicit tol statistics are produced by the hypothesis analysis script (see §3.6).
...
**Hypothesis analysis (H2, H3a).** A script `docs/scripts/analyze_hypotheses.py` reads `etl_log.csv` from one or more run directories and reports: **(H2)** tolerance statistics (min, max, median, fraction of chunks at tol_max or in the 4–8×10⁻³ band)—in adaptive-QEq runs (etl_qeq, etl_full) tol typically stays in a narrow range, consistent with the cap and tol_max limiting how loose *tol* can get; **(H3a)** when a QEq model log (or `--model-A` / `--model-B`) is provided, the script computes cap = α_qeq Δℓ² *k*ₑ*T* / dt² and predicted force error from the log-linear model, and reports the ratio predicted_err/cap and the fraction of chunks with predicted_err ≤ cap (formulation consistency). Run from the repo root, e.g. `python docs/scripts/analyze_hypotheses.py path/to/outputs_sio2_v4_hat_etl_full path/to/outputs_sio2_v4_hat_etl_qeq` (or pass directories containing `etl_log.csv`). Investigation runs (e.g. `--out-prefix investigation`) can be analyzed the same way; see `docs/Investigation_Runs.md` if present.
...
- **`reax_sandbox/etl_python/`** — ETL controller (`etl_controller.py`), LAMMPS run helpers, and shared utilities.
- **`reax_sandbox/sio2_etl/`** — SiO₂ suite: `run_suite_sio2.py` (seven-case suite per schedule), `make_silica_1500.py`, data/restart paths, and `README.md` for local run instructions.
- **`old_etl/`** — Standalone CHO reproduction script and notes (`run_cho_reproduce.py`, `README_REPRODUCE.md`).
- **`docs/`** — Full report (same as this README content) in `ETL_SiO2_Report.md`.
...
**Requirements.** Python 3 with `lammps` (pip install lammps), NumPy. See `requirements.txt`. LAMMPS must be built with ReaxFF and the Python interface. SiO₂ force field and initial structures are referenced from `reax_sandbox/sio2_etl/`.
