...
Additional analysis figures (simulated time vs wall time, controller behavior, fidelity trajectories, temperature schedule schematic, dashboards) are listed with captions in [Supplementary_Figures.md](Supplementary_Figures.md). All figures are produced from `etl_log.csv` and `wall_time.txt` in the suite output directories.
