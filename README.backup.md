...
The following main figures are generated by `docs/scripts/generate_figures.py` (run from repo root with the project venv: `python docs/scripts/generate_figures.py`). They are stored under `docs/figures/`.

**Fig. 1 — ETL control loop (schematic)**  
![Fig. 1: ETL control loop](docs/figures/fig01_control_loop.png)

**Fig. 2 — Virtual schedule clock**  
![Fig. 2: Virtual schedule clock](docs/figures/fig02_virtual_clock.png)

**Fig. 3 — Wall time by case and schedule**  
![Fig. 3: Wall time by case](docs/figures/fig03_wall_time.png)

**Fig. 4 — Speedup vs moderate baseline**  
![Fig. 4: Speedup vs moderate](docs/figures/fig04_speedup_vs_moderate.png)

Additional analysis figures (simulated time vs wall time, controller behavior, fidelity trajectories, temperature schedule schematic, dashboards) are listed with captions in [Supplementary_Figures.md](docs/Supplementary_Figures.md). All figures are produced from `etl_log.csv` and `wall_time.txt` in the suite output directories.
