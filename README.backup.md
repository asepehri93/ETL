...
Figures can be added to the report by placing image files (e.g. PNG or SVG) in the repository (e.g. under `docs/figures/`) and referencing them in Markdown, for example: `![Wall times by case](figures/walltime_576.png)`. The data in §3.1 (576-atom wall times), §3.7 (1500-atom wall times), and the speedup tables is sufficient to generate bar charts of wall time by case, speedup versus baseline by schedule, and comparison plots across system sizes. Conceptual diagrams (e.g. schematic of the virtual schedule clock, or the ETL control loop) can be produced separately and included in the same way. This report does not embed generated figures by default so that it remains readable as plain text; the tables provide the full numerical results needed to reproduce or extend any figures.
